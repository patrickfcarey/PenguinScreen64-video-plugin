...
struct Config
{
	...
	// VR (mupen64plus-VR). All-zero/off by default so a stock (non-VR) run is
	// byte-identical. enableVR is the master switch; when set it forces
	// video.threadedVideo = 0 (see Config_LoadConfig) so the head-pose store and
	// stereo params can stay plain unlocked structs shared on one thread.
	struct { u32 enableVR; f32 stereoSep; f32 stereoConv; f32 headLookGain; } vr;
