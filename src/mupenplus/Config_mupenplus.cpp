...
bool Config_SetDefault()
{
	...
	//#VR settings (mupen64plus-VR). Off by default; EnableVR forces ThreadedVideo=0.
	res = ConfigSetDefaultBool(g_configVideoGliden64, "EnableVR", config.vr.enableVR, "Enable VR support (virtual screen / stereo / head-look). Forces ThreadedVideo off.");
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultFloat(g_configVideoGliden64, "VrStereoSeparation", config.vr.stereoSep, "VR stereo separation (per-eye disparity). 0 = mono.");
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultFloat(g_configVideoGliden64, "VrStereoConvergence", config.vr.stereoConv, "VR stereo convergence (zero-parallax depth, in clip-w units).");
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultFloat(g_configVideoGliden64, "VrHeadLookGain", config.vr.headLookGain, "VR head-look rotation gain multiplier.");
	assert(res == M64ERR_SUCCESS);
	...
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultBool(g_configVideoGliden64, "FBInfoReadColorChunk", config.frameBufferEmulation.fbInfoReadColorChunk, "Read color buffer by 4kb chunks (strictly follow FBRead specification).");
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultBool(g_configVideoGliden64, "FBInfoReadDepthChunk", config.frameBufferEmulation.fbInfoReadDepthChunk, "Read depth buffer by 4kb chunks (strictly follow FBRead specification).");
	assert(res == M64ERR_SUCCESS);
	...
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultBool(g_configVideoGliden64, "txHiresFullAlphaChannel", config.textureFilter.txHiresFullAlphaChannel, "Allow to fully use the alpha channel for high resolution textures.");
	assert(res == M64ERR_SUCCESS);
	res = ConfigSetDefaultBool(g_configVideoGliden64, "txHresAltCRC", config.textureFilter.txHresAltCRC, "Use alternative method for paletted textures CRC calculation.");
	assert(res == M64ERR_SUCCESS);
...
void Config_LoadConfig()
{
	...
	//#VR settings (D3 enforcement happens at the end of this function, post-validate).
	config.vr.enableVR = ConfigGetParamBool(g_configVideoGliden64, "EnableVR");
	config.vr.stereoSep = ConfigGetParamFloat(g_configVideoGliden64, "VrStereoSeparation");
	config.vr.stereoConv = ConfigGetParamFloat(g_configVideoGliden64, "VrStereoConvergence");
	config.vr.headLookGain = ConfigGetParamFloat(g_configVideoGliden64, "VrHeadLookGain");
	...
	// D3: VR requires single-threaded video. The head-pose store and stereo params
	// are plain unlocked structs shared between the emulator thread and the GL path;
	// ThreadedVideo would move GL onto a render thread and reintroduce the cross-
	// thread hazard. Re-enabling ThreadedVideo later requires a locked/double-
	// buffered pose first. This is the config-read completion path: it runs before
	// DisplayWindowMupen64plus::_start() reads config.video.threadedVideo and calls
	// FunctionWrapper::setThreadedMode() (mupen64plus_DisplayWindow.cpp:82), reached
	// via RomOpen() -> Config_LoadConfig() -> dwnd().start() (and likewise the
	// RSPTHREAD path RSP_ThreadProc() -> Config_LoadConfig() -> dwnd().start(),
	// CommonAPIImpl_common.cpp:40-41 / :213-214).
	if (config.vr.enableVR != 0 && config.video.threadedVideo != 0) {
		LOG(LOG_WARNING, "VR enabled: forcing ThreadedVideo=0 (was 1).\n");
