...
m64p_error PluginAPI::PluginStartup(m64p_dynlib_handle _CoreLibHandle, void* Context, void (*DebugCallback)(void *, int, const char *))
{
	...
	/* Resolve the optional mupen64plus-VR head-pose export (packet S2). NULL
	   against a stock core -> VR pose import stays inert. Wiring it into the
	   render path is S17; here we only resolve + log the availability state. */
	vr_gliden64_resolve(_CoreLibHandle);
